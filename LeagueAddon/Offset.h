...
namespace Offset {
	namespace Data {
		static constexpr int LocalPlayer = 0x313AD80; // 51 8B 0D ? ? ? ? 85 C9 74 21 
		static constexpr int GameTime = 0x3134810; // F3 0F 11 05 ? ? ? ? 8B 49
		static constexpr int ObjectManager = 0x189E25C; // A1 ?? ?? ?? ?? C7 40 ?? ?? ?? ?? ?? C3
		static constexpr int HudInstance = 0x189E320; //8B ? ? ? ? ? 6A 00 8B ? ? E8 ? ? ? ? B0 01 ?
		static constexpr int Camera = 0x3131B60; // E8 ?? ?? ?? ?? 8B 0D ?? ?? ?? ?? 8D 43 24

		static constexpr int ManagerTemplate_Minions = 0x24EBAE4; // 8B 0D ? ? ? ? E8 ? ? ? ? EB 09 56 E8 ? ? ? ? 83 C4 04 8B CB C7 44 24 ? ? ? ? ?  First
		static constexpr int ManagerTemplate_Turrets = 0x3132FDC; // 51 53 55 56 8B F1 8D 44 24 0C 89 35 ? ? ? ? 
		static constexpr int ManagerTemplate_Heroes = 0x189E2F4; // 83 EC 64 A1 ? ? ? ? 33 C4 89 44 24 60 8B 44 24 68 83 CA FF 
		static constexpr int ManagerTemplate_Missiles = 0x313ADF8; // 8D 4E 04 C7 05 ? ? ? ? ? ? ? ? 
		...
		static constexpr int AntiCheatCheck = 0x1882CA8;
	...
	namespace Function {
		...
		static constexpr int GetAttackDelay = 0x2846C0; // 8B 44 24 04 51 F3
		static constexpr int GetAttackCastDelay = 0x2845C0; // 83 EC 0C 53 8B 5C 24 14 8B CB 56 57 8B 03 FF 90
		static constexpr int GetBoundingRadius = 0x152580; // 83 EC 08 56 8B F1 83 BE ? ? ? ? ? 74 69
		static constexpr int GetBasicAttack = 0x151D80; // E8 ? ? ? ? 6A 40 56 8B D8 

		static constexpr int IsMinion = 0x188DE0; // 56 8B 74 24 08 57 BF ? ? ? ? 85 F6 74 52 // E8 ? ? ? ? 83 C4 04 84 C0 74 6C
		static constexpr int IsTurret = 0x188ED0; // E8 ? ? ? ? 83 C4 04 84 C0 75 7A
		//static constexpr int IsAlive = 0x16AC50; // not used

		static constexpr int IssueOrder = 0x15D450; // E8 ? ? ? ? 8D 46 11 
		static constexpr int GetBaseDrawPosition = 0x154370; // E8 ?? ?? ?? ?? EB ?? 8B 01 8B 40

		static constexpr int CastSpell = 0x5E4130; // not used

		static constexpr int OnProcessSpell = 0x535110;
		// E8 ? ? ? ? 8B 9D ? ? ? ? 89 44 24 28 -- pattern work
		// E8 ? ? ? ? 8B AF ? ? ? ? 89 44 24 24 -- pattern not working
		// E8 ? ? ? ? 85 C0 0F 94 44 24 ?		-- pattern not working
		// E8 ? ? ? ? 8B 9D ? ? ? ? 89 44 24 28 -- pattern work

		static constexpr int PrintChat = 0x5E2E80; // E8 ? ? ? ? 6A 00 68 ? ? ? ? E8 ? ? ? ? 83 C4 04
		static constexpr int SendChat = 0x662190; // A1 ? ? ? ? 56 6A FF
		static constexpr int GetPing = 0x349910; // 8B C8 E8 ?? ?? ?? ?? 68 ?? ?? ?? ?? 8D 4D E0

		static constexpr int WorldToScreen = 0xA7FAD0; //8B 11 8D 41 04 50 
		// E8 ? ? ? ? 83 C4 08 83 F8 01 75 16 
		// E8 ? ? ? ? 83 C4 08 84 C0 75 3D 

		static constexpr int CharacterDataStack__Update = 0x107EE0; // 83 EC 1C 56 57 8D 44 24 ? 8B F1 50 8D 44 24 ? 50 8D 44 24 ? 50 8D 44 24 ? 50 8D 44 24 ? 50 8D 44 24 ? 50
		static constexpr int CharacterDataStack__Push = 0x118C90; // 83 EC 74 53 55 56 57 8B F9 8B 47 04 39 07

		static constexpr int IsNotWall = 0xA130D0; // E8 ? ? ? ? 33 C9 83 C4 0C 84
	}
	...
	namespace Minimap {
		static constexpr int Object = 0x3131B80; // 55 8B EC 83 E4 F0 81 EC ? ? ? ? A1 ? ? ? ? 33 C4 89 84 24 ? ? ? ? 8B 0D ? ? ? ? 0F 57 C0 8B 45 08 89 44 24 08 53 56 57 0F 11 84 24 ? ? ? ? 0F 11 84 24 ? ? ? ? 85 C9 74 1D 8D 84 24 ? ? ? ? 50 E8 ? ? ? ? 0F 10 84 24 ? ? ? ?  mov     ecx, dword_329A27C
	...
	namespace GameObject {
		...
		static constexpr int SpellBook = 0x2970; // 12.12 // 8B 84 83 ? ? ? ? EB 06 8B 83 ? ? ? ? 85 C0 0F 84 ? ? ? ? 53 8B CF E8 ? ? ? ? 8B C8 8B 10 FF 52 18 8B F0
		...
		static constexpr int ChampionName = 0x2D7C; // 12.12
		...
		static constexpr int CharacterDataStack = 0x2D70; // 8D 8E ? ? ? ? FF 74 24 4C

		static constexpr int RecallState = 0xD90;
		static constexpr int SkinPtr = 0x2D54; // 12.12
	...
	namespace Inventory
	{
		static constexpr int Instance = 0x35A8;
