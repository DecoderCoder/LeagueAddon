...
namespace Geometry
{
	...
	public:
		...
		Arc(const Vector3& start, const Vector3& end, int hitbox)
		{
			...
			End = end;
			HitBox = hitbox;
			distanceTo = Start.distanceTo(End);
		}
	...
	public:
		...
		static std::vector<Vector3> CutPath(std::vector<Vector3> path, float distanceTo)
		{
			...
			if (distanceTo < 0)
			{
				path[0] = path[0] + (path[1] - path[0]).Normalized() * distanceTo;
				return path;
			}

			for (auto i = 0; i < path.size() - 1; i++)
			{
				auto dist = path[i].distanceTo(path[i + 1]);
				if (dist > distanceTo)
				{
					result.push_back(path[i] + (path[i + 1] - path[i]).Normalized() * distanceTo);
		...
		static float PathLength(std::vector<Vector3> path)
		{
			...
			{
				distanceTo += path[i].distanceTo(path[i + 1]);
			}
			return distanceTo;
		}
		...
		static Vector3 PositionAfter(std::vector<Vector3> self, int t, int speed, int delay = 0)
		{
			...
			for (int i = 0; i <= self.size() - 2; i++)
			{
				Vector3 from = self[i];
				Vector3 to = self[i + 1];
				int d = (int)to.distanceTo(from);
				if (d > distanceTo)
				{
					return from + (to - from).Normalized() * distanceTo;
				}
				distanceTo -= d;
			}
