...
static std::vector<std::unique_ptr<PLH::x86Detour>> hooks;
static std::unique_ptr<PLH::x86Detour> detour;
...
kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			windowClass.lpszClassName = KIERO_TEXT("LAddon");
			windowClass.hIconSm = NULL;

			::RegisterClassEx(&windowClass);

			HWND window = ::CreateWindow(windowClass.lpszClassName, KIERO_TEXT("LAddon"), WS_OVERLAPPEDWINDOW, 0, 0, 100, 100, NULL, NULL, windowClass.hInstance, NULL);
			...
			else if (_renderType == RenderType::D3D11)
			{
				...
				detour.reset(new PLH::x86Detour((char*)target, (char*)&DirectXHook::Hooked_PresentDX11, (uint64_t*)&DirectXHook::oPresentDX11, dis));
				detour->hook();

				///*if (detour->hook())
				//	MessageBoxA(0, "Hooked", "lol", 0);
				//else
				//	MessageBoxA(0, "Failed", "lol", 0);*/
...
#endif
		void* target = (void*)g_methodsTable[_index];
		MessageBoxA(0, to_hex((int)target).c_str(), "", 0);
		std::unique_ptr<PLH::x86Detour> det;

		det.reset(new PLH::x86Detour((char*)target, (char*)_function, (uint64_t*)_original, dis));
